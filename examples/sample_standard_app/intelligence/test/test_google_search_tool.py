...
class GoogleSearchToolTest(unittest.TestCase):
    ...
    def test_google_search_mock_mode(self):
        """测试Google搜索工具模拟模式"""
        # 不设置API密钥，测试模拟模式
        result = self.search_tool.execute(input="测试查询")
        self.assertIsInstance(result, str)
        self.assertIn("Google 搜索模拟结果", result)
    ...
    def test_google_scholar_search_mock_mode(self):
        """测试Google学术搜索工具模拟模式"""
        result = self.scholar_tool.execute(input="机器学习")
        self.assertIsInstance(result, str)
        self.assertIn("Google学术搜索模拟结果", result)
