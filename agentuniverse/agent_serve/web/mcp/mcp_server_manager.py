...
def is_method_overridden(override_method: callable, origin_method: callable) -> bool:
...
def create_exposed_tool_method_wrapper(tool_name):
    ...
    if not is_method_overridden(original_method, getattr(Tool, method_name)):
        method_name = "execute"
        original_method = getattr(ToolManager().get_instance_obj(tool_name).__class__, method_name)
    ...
    exposed_signature = inspect.Signature(
        parameters=exposed_parameters, return_annotation=original_signature.return_annotation
    )
...
class MCPServerManager:
    server_tool_map: dict = {DEFAULT_SERVER_NAME: {"tool": [], "toolkit": []}}
    ...
    def register_mcp_tool(self, configer_instance: ComponentConfiger, configer_type: str):
        if isinstance(configer_instance.as_mcp_tool, dict):
            ...
            if server_name not in self.server_tool_map:
                self.server_tool_map[server_name] = {"tool": [], "toolkit": []}
    ...
    def start_server(
        self, host: str = "0.0.0.0", port: int = 8890, transport: Literal["sse", "streamable_http"] = "sse"
    ):
        import contextlib
        import uvicorn
        ...
        for server_name, tool_dict in self.server_tool_map.items():
            ...
            mcp_server_dict = {"server_name": server_name, "server": mcp_server}
            if transport == "sse":
