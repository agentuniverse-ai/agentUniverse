...
class RequestLibrary:
    def __init__(self, configer: Configer = None):
        ...
        self.sqldb_wrapper = SQLDBWrapper(name="__system_db__", db_wrapper_configer=_configer)
        SQLDBWrapperManager().register(self.sqldb_wrapper.get_instance_code(), self.sqldb_wrapper)
    ...
    def query_request_by_request_id(self, request_id: str) -> RequestDO | None:
        ...
        try:
            result = (
                session.execute(select(self.request_orm).where(self.request_orm.request_id == request_id))
                .scalars()
                .first()
            )
            if not result:
    ...
    def update_request(self, request_do: RequestDO):
        ...
        try:
            db_request_do = (
                session.query(self.request_orm).filter(self.request_orm.request_id == request_do.request_id).first()
            )
            if db_request_do:
    ...
    def update_gmt_modified(self, request_id: str):
        ...
        session = self.get_session()
        try:
            db_request_do = session.query(self.request_orm).filter(self.request_orm.request_id == request_id).first()
            if db_request_do:
