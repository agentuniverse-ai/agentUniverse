...
from agentuniverse.base.annotation.trace import _get_tool_info, InvocationChainContext
...
class ToolMetricsRecorder:
    ...
    def record_total_token_usage(self, labels: Dict[str, str]) -> None:
        ...
        self.metrics[MetricNames.TOOL_COMPLETION_TOKENS].record(get_current_token_usage().completion_tokens, labels)
        self.metrics[MetricNames.TOOL_REASONING_TOKENS].record(get_current_token_usage().reasoning_tokens, labels)
        self.metrics[MetricNames.TOOL_CACHED_TOKENS].record(get_current_token_usage().cached_tokens, labels)
...
class ToolSpanAttributesSetter:
    ...
    def set_input_attributes(
        ...
        span.set_attribute(SpanAttributes.TOOL_INPUT, safe_json_dumps(input_params, ensure_ascii=False))
    ...
    @staticmethod
    def set_success_attributes(span: Span, duration: float, result: Any) -> None:
        ...
        span.set_attribute(SpanAttributes.TOOL_OUTPUT, safe_json_dumps(result, ensure_ascii=False))
        span.set_attribute(SpanAttributes.TOOL_USAGE_TOTAL_TOKENS, get_current_token_usage().total_tokens)
        span.set_attribute(SpanAttributes.TOOL_USAGE_COMPLETION_TOKENS, get_current_token_usage().completion_tokens)
        span.set_attribute(SpanAttributes.TOOL_USAGE_PROMPT_TOKENS, get_current_token_usage().prompt_tokens)
        span.set_attribute(
            SpanAttributes.TOOL_USAGE_DETAIL_TOKENS, safe_json_dumps(get_current_token_usage().to_dict())
        )
...
class ToolInstrumentor(BaseInstrumentor):
    ...
    def _uninstrument(self, **kwargs):
        ...
        try:
            ...
            setattr(trace_module, "_tool_wrapper_async", self._original_tool_wrapper_async)
            setattr(trace_module, "_tool_wrapper_sync", self._original_tool_wrapper_sync)
        except ImportError:
            pass
    ...
    def _setup_metrics(self):
        ...
        self._metrics = {
            MetricNames.TOOL_CALLS_TOTAL: self._meter.create_counter(
                name=MetricNames.TOOL_CALLS_TOTAL, description="Total number of Tool calls", unit="1"
            ),
            MetricNames.TOOL_ERRORS_TOTAL: self._meter.create_counter(
                name=MetricNames.TOOL_ERRORS_TOTAL, description="Total number of Tool errors", unit="1"
            ),
            MetricNames.TOOL_CALL_DURATION: self._meter.create_histogram(
                name=MetricNames.TOOL_CALL_DURATION, description="Duration of Tool calls in seconds", unit="s"
            ),
            MetricNames.TOOL_TOTAL_TOKENS: self._meter.create_histogram(
                name=MetricNames.TOOL_TOTAL_TOKENS, description="Total token used in tool call", unit="1"
            ),
            MetricNames.TOOL_COMPLETION_TOKENS: self._meter.create_histogram(
                name=MetricNames.TOOL_COMPLETION_TOKENS, description="Completion token used in tool call", unit="1"
            ),
            MetricNames.TOOL_PROMPT_TOKENS: self._meter.create_histogram(
                name=MetricNames.TOOL_PROMPT_TOKENS, description="Prompt token used in tool call", unit="1"
            ),
            MetricNames.TOOL_CACHED_TOKENS: self._meter.create_histogram(
                name=MetricNames.TOOL_CACHED_TOKENS, description="Cached token used in tool call", unit="1"
            ),
            MetricNames.TOOL_REASONING_TOKENS: self._meter.create_histogram(
                name=MetricNames.TOOL_REASONING_TOKENS, description="Reasoning token used in tool call", unit="1"
            ),
        }
    ...
    def _patch_trace_tool(self):
        ...
        try:
            ...
            self._original_tool_wrapper_async = getattr(trace_module, "_tool_wrapper_async", None)
            self._original_tool_wrapper_sync = getattr(trace_module, "_tool_wrapper_sync", None)
            setattr(trace_module, "_tool_wrapper_async", self._wrap_async_tool_function)
            setattr(trace_module, "_tool_wrapper_sync", self._wrap_sync_tool_function)
        except ImportError:
            pass
    ...
    async def _wrap_async_tool_function(self, func, *args, **kwargs):
        ...
        with InvocationChainContext(source=source, node_type="tool"):
            try:
                ToolSpanAttributesSetter.set_input_attributes(span, source, tool_input, start_info, pair_id)
                ...
                ToolSpanAttributesSetter.set_success_attributes(span, duration, result)
                ...
                self._metrics_recorder.record_total_token_usage(labels)
                ToolSpanAttributesSetter.set_error_attributes(span_manager.span, e, duration)
                raise
