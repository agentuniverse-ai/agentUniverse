...
from agentuniverse.base.annotation.trace import _get_agent_info, InvocationChainContext
...
from .consts import INSTRUMENTOR_NAME, INSTRUMENTOR_VERSION, MetricNames, MetricLabels, SpanAttributes
...
class AgentMetricsRecorder:
    ...
    def record_first_token(self, duration: float, labels: Dict[str, str]) -> None:
        """Record start of Agent call."""
    ...
    def record_total_token_usage(self, labels: Dict[str, str]) -> None:
        """Record start of Agent call."""
        self.metrics[MetricNames.AGENT_TOTAL_TOKENS].record(get_current_token_usage().total_tokens, labels)
        self.metrics[MetricNames.AGENT_COMPLETION_TOKENS].record(get_current_token_usage().completion_tokens, labels)
        self.metrics[MetricNames.AGENT_PROMPT_TOKENS].record(get_current_token_usage().prompt_tokens, labels)
        self.metrics[MetricNames.AGENT_CACHED_TOKENS].record(get_current_token_usage().cached_tokens, labels)
        self.metrics[MetricNames.AGENT_REASONING_TOKENS].record(get_current_token_usage().reasoning_tokens, labels)
...
class AgentSpanAttributesSetter:
    ...
    def set_input_attributes(
        span: Span, source_name: str, input_params: Dict[str, Any], caller_info: Dict[str, Any], pair_id: str
    ) -> None:
        ...
        span.set_attribute(SpanAttributes.AGENT_INPUT, safe_json_dumps(input_params, ensure_ascii=False))
    ...
    @staticmethod
    def set_success_attributes(span: Span, duration: float, result: OutputObject) -> None:
        ...
        span.set_attribute(SpanAttributes.AGENT_OUTPUT, safe_json_dumps(result.to_dict(), ensure_ascii=False))
        span.set_attribute(SpanAttributes.AGENT_USAGE_TOTAL_TOKENS, get_current_token_usage().total_tokens)
        span.set_attribute(SpanAttributes.AGENT_USAGE_COMPLETION_TOKENS, get_current_token_usage().completion_tokens)
        span.set_attribute(SpanAttributes.AGENT_USAGE_PROMPT_TOKENS, get_current_token_usage().prompt_tokens)
        span.set_attribute(
            SpanAttributes.AGENT_USAGE_DETAIL_TOKENS, safe_json_dumps(get_current_token_usage().to_dict())
        )
...
class AgentInstrumentor(BaseInstrumentor):
    ...
    def _uninstrument(self, **kwargs):
        ...
        try:
            ...
            setattr(trace_module, "_agent_wrapper_async", self._original_agent_wrapper_async)
            setattr(trace_module, "_agent_wrapper_sync", self._original_agent_wrapper_sync)
        except ImportError:
            pass
    ...
    def _setup_metrics(self):
        ...
        self._metrics = {
            MetricNames.AGENT_CALLS_TOTAL: self._meter.create_counter(
                name=MetricNames.AGENT_CALLS_TOTAL, description="Total number of Agent calls", unit="1"
            ),
            MetricNames.AGENT_ERRORS_TOTAL: self._meter.create_counter(
                name=MetricNames.AGENT_ERRORS_TOTAL, description="Total number of Agent errors", unit="1"
            ),
            MetricNames.AGENT_CALL_DURATION: self._meter.create_histogram(
                name=MetricNames.AGENT_CALL_DURATION, description="Duration of Agent calls in seconds", unit="s"
            ),
            ...
            MetricNames.AGENT_TOTAL_TOKENS: self._meter.create_histogram(
                name=MetricNames.AGENT_TOTAL_TOKENS, description="Total token nums used in agent", unit="1"
            ),
            MetricNames.AGENT_COMPLETION_TOKENS: self._meter.create_histogram(
                name=MetricNames.AGENT_COMPLETION_TOKENS, description="Completion token nums used in agent", unit="1"
            ),
            MetricNames.AGENT_PROMPT_TOKENS: self._meter.create_histogram(
                name=MetricNames.AGENT_PROMPT_TOKENS, description="Prompt token nums used in agent", unit="1"
            ),
            MetricNames.AGENT_CACHED_TOKENS: self._meter.create_histogram(
                name=MetricNames.AGENT_CACHED_TOKENS, description="Cached token nums used in agent", unit="1"
            ),
            MetricNames.AGENT_REASONING_TOKENS: self._meter.create_histogram(
                name=MetricNames.AGENT_REASONING_TOKENS, description="Reasoning token nums used in agent", unit="1"
            ),
        }
    ...
    def _patch_trace_agent(self):
        ...
        try:
            ...
            self._original_agent_wrapper_async = getattr(trace_module, "_agent_wrapper_async", None)
            self._original_agent_wrapper_sync = getattr(trace_module, "_agent_wrapper_sync", None)
            setattr(trace_module, "_agent_wrapper_async", self._wrap_async_agent_function)
            setattr(trace_module, "_agent_wrapper_sync", self._wrap_sync_agent_function)
        except ImportError:
            pass

    def _wrap_output_stream(self, output_stream: Any, start_time: float, span: Span, labels: Dict[str, str]) -> Any:
        ...
        if hasattr(asyncio, "Queue") and isinstance(output_stream, asyncio.Queue):
    ...
    async def _wrap_async_agent_function(self, func, *args, **kwargs):
        ...
        with InvocationChainContext(source=source, node_type="agent"):
            try:
                AgentSpanAttributesSetter.set_input_attributes(span, source, agent_input, start_info, pair_id)
                ...
                if "output_stream" in kwargs and kwargs["output_stream"] is not None:
                    # Add streaming label
                ...
                if not labels[MetricLabels.STREAMING]:
                    self._metrics_recorder.record_first_token(duration, labels)
                    AgentSpanAttributesSetter.set_first_token_attributes(span, duration)
                AgentSpanAttributesSetter.set_success_attributes(span, duration, result)
                ...
                self._metrics_recorder.record_total_token_usage(labels)
                AgentSpanAttributesSetter.set_error_attributes(span_manager.span, e, duration)
                raise
