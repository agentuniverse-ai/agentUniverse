...
from .consts import INSTRUMENTOR_NAME, INSTRUMENTOR_VERSION, MetricNames, SpanAttributes, MetricLabels
...
def safe_json_dumps(obj, *, indent=None, ensure_ascii=False) -> str:
    ...
    try:
        return json.dumps(obj, default=_fallback, indent=indent, ensure_ascii=ensure_ascii)
    except Exception:
        return str(obj)
...
class LLMSpanManager:
    ...
    def start_span(self) -> Span:
        ...
        self.span = self.tracer.start_span(
            name=self.span_name, kind=trace.SpanKind.INTERNAL, context=context.get_current()
        )
    ...
    def cleanup(self):
        ...
        if self.span:
            try:
                add_current_token_usage_to_parent(
                    get_current_token_usage(self.span.context.span_id), self.span.parent.span_id
                )
            except:
                pass
            self.span.end()
...
class LLMMetricsRecorder:
    ...
    def record_first_token(self, duration: float, labels: Dict[str, str], streaming: bool = True) -> None:
    ...
    def record_token_usage(self, usage: TokenUsage, labels: Dict[str, str]) -> None:
        """Record token usage metrics."""
        if usage:
            self.metrics[MetricNames.LLM_PROMPT_TOKENS].record(usage.prompt_tokens, labels)
            self.metrics[MetricNames.LLM_COMPLETION_TOKENS].record(usage.completion_tokens, labels)
            self.metrics[MetricNames.LLM_TOTAL_TOKENS].record(usage.total_tokens, labels)
            self.metrics[MetricNames.LLM_CACHED_TOKENS].record(usage.cached_tokens, labels)
            self.metrics[MetricNames.LLM_REASONING_TOKENS].record(usage.reasoning_tokens, labels)

    def record_error(self, error: Exception, duration: float, labels: Dict[str, str]) -> None:
...
class LLMSpanAttributesSetter:
    ...
    def set_input_attributes(
        ...
        span.set_attribute(SpanAttributes.AU_LLM_INPUT, safe_json_dumps(input_params, ensure_ascii=False))
        span.set_attribute(SpanAttributes.AU_LLM_LLM_PARAMS, safe_json_dumps(llm_params, ensure_ascii=False))
        span.set_attribute(SpanAttributes.AU_TRACE_CALLER_NAME, caller_info.get("source", "unknown"))
        span.set_attribute(SpanAttributes.AU_TRACE_CALLER_TYPE, caller_info.get("type", "user"))

    @staticmethod
    def set_success_attributes(span: Span, duration: float, result: LLMOutput) -> None:
        ...
        if result.usage:
            span.set_attribute(SpanAttributes.AU_LLM_USAGE_PROMPT_TOKENS, result.usage.prompt_tokens)
            span.set_attribute(SpanAttributes.AU_LLM_USAGE_COMPLETION_TOKENS, result.usage.completion_tokens)
            span.set_attribute(SpanAttributes.AU_LLM_USAGE_TOTAL_TOKENS, result.usage.total_tokens)
            span.set_attribute(SpanAttributes.AU_LLM_USAGE_DETAIL_TOKENS, safe_json_dumps(result.usage.to_dict()))

    @staticmethod
    def set_error_attributes(span: Span, error: Exception, duration: float) -> None:
...
class StreamingResultProcessor:
    ...
    async def process_async_stream(self, result: AsyncGenerator) -> AsyncGenerator:
        ...
        try:
            async for chunk in result:
                if first_token:
                    ...
                    self.metrics_recorder.record_first_token(duration, self.labels)
                    LLMSpanAttributesSetter.set_first_token_attributes(self.span, duration)
    ...
    def process_sync_stream(self, result) -> Generator:
        ...
        try:
            for chunk in result:
                ...
                llm_output.append(chunk.text if hasattr(chunk, "text") else str(chunk))
    ...
    def _finalize_streaming_result(self, llm_output: list, usage: Optional[TokenUsage]) -> None:
        """Finalize streaming result processing."""
        output_str = "".join(llm_output)
        duration = time.time() - self.start_time

        Monitor().trace_llm_invocation(
            source=self.source, llm_input=self.llm_input, llm_output=output_str, cost_time=duration
        )

        pseudo_result = LLMOutput(text=output_str, usage=usage)
        Monitor().trace_llm_token_usage(self.llm_instance, self.llm_input, pseudo_result)

        # Record success metrics
        self.metrics_recorder.record_duration(duration, self.labels)
        if pseudo_result.usage:
            self.metrics_recorder.record_token_usage(pseudo_result.usage, self.labels)
        LLMSpanAttributesSetter.set_success_attributes(self.span, duration, pseudo_result)

    def _handle_streaming_error(self, error: Exception, duration: float) -> None:
        """Handle streaming error."""
        self.metrics_recorder.record_error(error, duration, self.labels)
        LLMSpanAttributesSetter.set_error_attributes(self.span, error, duration)
...
class LLMInstrumentor(BaseInstrumentor):
    ...
    def _instrument(self, **kwargs):
        ...
        self._tracer = trace.get_tracer(INSTRUMENTOR_NAME, INSTRUMENTOR_VERSION, tracer_provider)
        self._meter = metrics.get_meter(INSTRUMENTOR_NAME, INSTRUMENTOR_VERSION, meter_provider)
    ...
    def _uninstrument(self, **kwargs):
        ...
        try:
            ...
            setattr(trace_module, "_llm_wrapper_async", self._original_llm_wrapper_async)
            setattr(trace_module, "_llm_wrapper_sync", self._original_llm_wrapper_sync)
        except ImportError:
            pass
    ...
    def _setup_metrics(self):
        ...
        self._metrics = {
            MetricNames.LLM_CALLS_TOTAL: self._meter.create_counter(
                name=MetricNames.LLM_CALLS_TOTAL, description="Total number of LLM calls", unit="1"
            ),
            MetricNames.LLM_ERRORS_TOTAL: self._meter.create_counter(
                name=MetricNames.LLM_ERRORS_TOTAL, description="Total number of LLM errors", unit="1"
            ),
            MetricNames.LLM_CALL_DURATION: self._meter.create_histogram(
                name=MetricNames.LLM_CALL_DURATION, description="Duration of LLM calls in seconds", unit="s"
            ),
            ...
            MetricNames.LLM_TOTAL_TOKENS: self._meter.create_histogram(
                name=MetricNames.LLM_TOTAL_TOKENS, description="Number of tokens used in LLM calls", unit="1"
            ),
            MetricNames.LLM_PROMPT_TOKENS: self._meter.create_histogram(
                name=MetricNames.LLM_PROMPT_TOKENS, description="Number of prompt tokens in LLM calls", unit="1"
            ),
            MetricNames.LLM_COMPLETION_TOKENS: self._meter.create_histogram(
                name=MetricNames.LLM_COMPLETION_TOKENS, description="Number of completion tokens in LLM calls", unit="1"
            ),
            MetricNames.LLM_REASONING_TOKENS: self._meter.create_histogram(
                name=MetricNames.LLM_REASONING_TOKENS, description="Number of reasoning tokens in LLM calls", unit="1"
            ),
            MetricNames.LLM_CACHED_TOKENS: self._meter.create_histogram(
                name=MetricNames.LLM_CACHED_TOKENS, description="Number of cached tokens in LLM calls", unit="1"
            ),
        }
    ...
    def _prepare_llm_context(self, func, *args, **kwargs) -> tuple:
        ...
        llm_instance, source, channel_name, llm_input, params, caller_info = _get_llm_info(func, *args, **kwargs)
    ...
    def _handle_non_streaming_result(
        self,
        result: LLMOutput,
        llm_instance,
        source: str,
    ...
    async def _wrap_async_llm_function(self, func, *args, **kwargs):
        ...
        llm_instance, source, channel_name, llm_input, params, caller_info, labels = self._prepare_llm_context(
            func, *args, **kwargs
        )
        ...
        try:
            ...
            LLMSpanAttributesSetter.set_input_attributes(span, source, channel_name, llm_input, params, caller_info)
            ...
            if isinstance(result, LLMOutput):
                ...
                self._handle_non_streaming_result(result, llm_instance, source, llm_input, duration, span, labels)
                ...
                processor = StreamingResultProcessor(
                    source, llm_input, llm_instance, start_time, span_manager, labels, self._metrics_recorder
                )
            ...
            LLMSpanAttributesSetter.set_error_attributes(span_manager.span, e, duration)
            span_manager.cleanup()
            raise
