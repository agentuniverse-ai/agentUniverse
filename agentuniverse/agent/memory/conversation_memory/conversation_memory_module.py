...
class ConversationMemoryModule:
    ...
    def _add_trace_info(
        self, source: str, source_type: str, target: str, target_type: str, type: str, params: dict, **kwargs
    ) -> None:
    ...
    def _add_trace(
        self, start_info, target_info: dict, type: str, params: dict, session_id: str, trace_id: str, pair_id: str
    ):
        if "kwargs" in params:
        ...
        kwargs = {
            "source": start_info["source"],
            "source_type": start_info["type"],
            "target": target_info["source"],
    ...
    def add_trace_info(self, start_info: dict, target_info: dict, type: str, params: dict, pair_id: str):
        ...
        def add_trace():
            self._add_trace(start_info, target_info, type, params, session_id, trace_id, pair_id)
    ...
    def add_knowledge_output_info(
        self, start_info: dict, target: str, params: List[Document], pair_id: str, auto: bool = True
    ):
        if not self.collection_current_agent_memory(start_info, "knowledge", auto):
            return
        target_info = {"source": target, "type": "knowledge"}
        ...
        self.add_trace_info(
            start_info, target_info, "output", {"output": "\n==============================\n".join(doc_data)}, pair_id
        )

    def add_agent_input_info(self, start_info: dict, instance: "Agent", params: dict, pair_id: str, auto: bool = True):
        if auto:
    ...
    def add_agent_result_info(
        self,
        agent_instance: "Agent",
        agent_result: Optional[OutputObject | dict],
        target_info: dict,
        pair_id: str,
        auto: bool = True,
    ):
        ...
        def add_trace():
            ...
            if auto:
                ...
                params = agent_result
            start_info = {"source": agent_instance.agent_model.info.get("name"), "type": "agent"}
    ...
    def add_llm_output_info(self, start_info: dict, target: str, output: str, pair_id: str, auto=True):
        if not self.collection_current_agent_memory(start_info, "llm", auto):
            return
        target_info = {"source": target, "type": "llm"}
        self.add_trace_info(start_info, target_info, "output", {"output": output}, pair_id)
