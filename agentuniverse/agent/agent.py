...
class Agent(ComponentBase, ABC):
    ...
    Provides core agent functionality including input/output parsing,
    execution, memory management, tool invocation, and knowledge retrieval.
    Subclasses must implement abstract methods for input/output key definitions
    and parsing logic.
    """
    ...
    def update_trace_context(self, input_object: InputObject):
        """Update trace context with session_id and trace_id from input object.

        Args:
            input_object (InputObject): Input parameters containing session_id and trace_id.
        """
    ...
    async def async_run(self, **kwargs) -> OutputObject:
        """Asynchronous agent instance running entry.

        Args:
            **kwargs: Variable keyword arguments for agent execution.
    ...
    async def async_execute(self, input_object: InputObject, agent_input: dict) -> dict:
        """Execute agent instance asynchronously.

        Args:
            input_object (InputObject): Input parameters passed by the user.
            agent_input (dict): Agent input parsed from input_object.
    ...
    def process_llm(self, **kwargs) -> LLM:
        """Process and initialize LLM instance based on configuration.

        Args:
            **kwargs: Optional keyword arguments, may contain 'llm_name'.
    ...
    def process_memory(self, agent_input: dict, **kwargs) -> Memory | None:
        """Process and initialize memory instance with chat history.

        Args:
            agent_input (dict): Agent input containing chat_history.
            **kwargs: Optional keyword arguments, may contain 'memory_name',
                      'conversation_memory', 'llm_name'.

        Returns:
            Memory | None: Initialized memory instance or None if no memory configured.
        """
    ...
    def invoke_chain(self, chain: RunnableSerializable[Any, str], agent_input: dict, input_object: InputObject,
                     **kwargs):
        """Invoke a LangChain runnable chain with streaming support.

        Args:
            chain (RunnableSerializable): LangChain runnable chain to execute.
            agent_input (dict): Agent input for the chain.
            input_object (InputObject): Input object containing output_stream config.
            **kwargs: Additional keyword arguments.

        Returns:
            str | dict: Chain execution result, either string or dict with text and reasoning_content.
        """
    ...
    async def async_invoke_chain(self, chain: RunnableSerializable[Any, str], agent_input: dict,
                                 input_object: InputObject, **kwargs):
        """Asynchronously invoke a LangChain runnable chain with streaming support.
    ...
    def judge_chain_stream(self,
        ...
        """Check if the chain supports streaming output.

        Args:
            chain (RunnableSerializable): LangChain runnable chain to check.

        Returns:
            bool: True if chain supports streaming, False otherwise.
        """
        streaming = False
    ...
    def generate_result(self, data: list[dict | str]):
        """Generate final result from streaming tokens.

        Args:
            data (list[dict | str]): List of tokens from streaming output.

        Returns:
            str | dict: Concatenated string result if input is list of strings,
                       otherwise dict with 'text' and 'reasoning_content' keys.
        """
    ...
    def invoke_tools(self, input_object: InputObject, **kwargs) -> str:
        """Invoke multiple tools and collect their results.

        Args:
            input_object (InputObject): Input object containing tool parameters.
            **kwargs: Optional keyword arguments, may contain 'tool_names'.

        Returns:
            str: Concatenated results from all tools, separated by double newlines.
        """
    ...
    async def async_invoke_tools(self, input_object: InputObject, **kwargs) -> str:
        """Asynchronously invoke multiple tools and collect their results.
    ...
    def invoke_knowledge(self, query_str: str, input_object: InputObject, **kwargs) -> str:
        """Query knowledge bases and retrieve relevant documents.

        Args:
            query_str (str): Query string for knowledge retrieval.
            input_object (InputObject): Input object containing query parameters.
            **kwargs: Optional keyword arguments, may contain 'knowledge_names'.

        Returns:
            str: Concatenated knowledge results from all knowledge bases,
    ...
    def process_prompt(self, agent_input: dict, **kwargs) -> ChatPrompt:
        """Process and build chat prompt from agent profile and prompt version.

        Args:
            agent_input (dict): Agent input containing expert_framework, image_urls, audio_url.
            **kwargs: Optional keyword arguments, may contain 'prompt_version'.

        Returns:
            ChatPrompt: Built chat prompt with introduction, target, and instruction.
        """
    ...
    def get_memory_params(self, agent_input: dict) -> dict:
        """Get memory retrieval parameters from agent model and input.

        Args:
            agent_input (dict): Agent input containing session_id and input.

        Returns:
            dict: Memory retrieval parameters including session_id, agent_id,
                  prune, top_k, memory_types, input, and type.
        """
    ...
    def get_run_config(self, **kwargs) -> dict:
        """Get LangChain runnable configuration with callbacks.

        Args:
            **kwargs: Optional keyword arguments, may contain 'llm_name'.

        Returns:
            dict: RunnableConfig with InvokeCallbackHandler callbacks.
        """
        llm_name = kwargs.get('llm_name') or self.agent_model.profile.get('llm_model', {}).get('name')
    ...
    def collect_current_memory(self, collect_type: str) -> bool:
        """Check if current memory should be collected based on collection types.

        Args:
            collect_type (str): Type of memory collection to check.

        Returns:
            bool: True if memory should be collected, False otherwise.
        """
    ...
    def load_memory(self, memory, agent_input: dict):
        """Load memory messages and convert to string format.

        Args:
            memory (Memory): Memory instance to load from.
            agent_input (dict): Agent input to store memory string.

        Returns:
            str: Memory string representation or default message if no memory.
        """
        if memory:
    ...
    def add_memory(self, memory: Memory, content: Any, type: str = 'Q&A', agent_input: dict[str, Any] = {}):
        ...
        Args:
            memory (Memory): Memory instance to add message to.
            content (Any): Message content to store.
            type (str): Message type, default is 'Q&A'.
            agent_input (dict): Agent input containing session_id.
        """
        if not memory:
            return
        session_id = agent_input.get('session_id')
    ...
    def summarize_memory(self, agent_input: dict[str, Any] = {}, memory: Memory = None):
        """Summarize memory content in a background thread.

        Args:
            agent_input (dict): Agent input containing session_id and other parameters.
            memory (Memory): Memory instance to summarize.
        """
    ...
    def load_summarize_memory(self, memory: Memory, agent_input: dict[str, Any] = {}) -> str:
        """Load summarized memory content.
        ...
        Returns:
            str: Summarized memory content or default message if no summarize memory exists.
        """
        if memory:
            params = self.get_memory_params(agent_input)
    ...
    def get_tool_descriptions(self) -> List[str]:
        """Get descriptions of all tools and toolkits configured for this agent.

        Returns:
            List[str]: List of tool descriptions including name and description.
        """
        description_list = []
    ...
    def get_func_call_list(self) -> List[str]:
        """Get function call list for this agent.

        Returns:
            List[str]: List of function names that can be called by this agent.
        """
        pass
    ...
    def create_copy(self):
        ...
        Returns:
            Agent: Deep copied agent instance with copied agent_model.
        """
