...
class Agent(ComponentBase, ABC):
    ...
    def judge_stream(self, llm: LLM, **kwargs):
        if llm._channel_instance:
            return kwargs.get('streaming', llm._channel_instance.channel_model_config.get('streaming', False))
        else:
            return kwargs.get('streaming', llm.streaming or False)
    ...
    def load_memory(self, memory, agent_input: dict) -> list[Message] | None:
        if not memory:
            return None
        params = self.get_memory_params(agent_input)
        LOGGER.info(f"Load memory with params: {params}")
        memory_messages: list[Message] = memory.get(**params)

        # parse input memory to message
    ...
    def _parse_input_mem_to_message(self, input_memory: Any) -> list:
        ...
        if not input_memory:
            return []
        if isinstance(input_memory, str):
            base.append(
                Message(
                    type=ChatMessageEnum.USER,
                    content=input_memory
                )
            )
        elif isinstance(input_memory, dict):
            base.append(Message.from_dict(input_memory))
        elif isinstance(input_memory, list):
            for _msg_dict in input_memory:
                base.append(Message.from_dict(_msg_dict))
        else:
