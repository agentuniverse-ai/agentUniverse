...
class Agent(ComponentBase, ABC):
    ...
    def get_instance_code(self) -> str:
        ...
        return f"{appname}.{self.component_type.value.lower()}.{name}"
    ...
    def initialize_by_component_configer(self, component_configer: AgentConfiger) -> "Agent":
        ...
        agent_model: Optional[AgentModel] = AgentModel(
            info=info, profile=profile, plan=plan, memory=memory, action=action
        )
    ...
    def process_llm(self, **kwargs) -> LLM:
        ...
        if is_system_builtin(llm):
            LOGGER.warn(
                "The system built-in LLM configuration YAML will be removed in the next version. "
    ...
    def process_memory(self, agent_input: dict, **kwargs) -> Memory | None:
        ...
        conversation_memory: Memory = MemoryManager().get_instance_obj(component_instance_name=conversation_memory_name)
    ...
    def invoke_chain(
        self, chain: RunnableSerializable[Any, str], agent_input: dict, input_object: InputObject, **kwargs
    ):
        ...
        for token in chain.stream(input=agent_input, config=self.get_run_config()):
            stream_output(
                input_object.get_data("output_stream", None),
                {"type": "token", "data": {"chunk": token, "agent_info": self.agent_model.info}},
            )
            result.append(token)
        return self.generate_result(result)

    async def async_invoke_chain(
        self, chain: RunnableSerializable[Any, str], agent_input: dict, input_object: InputObject, **kwargs
    ):
        if not self.judge_chain_stream(chain):
    ...
    def generate_result(self, data: list[dict | str]):
        ...
        return {"text": "".join(text), "reasoning_content": "".join(reasoning_content)}
    ...
    def invoke_knowledge(self, query_str: str, input_object: InputObject, **kwargs) -> str:
        ...
        for knowledge_name in knowledge_names:
            ...
            if knowledge is None:
                continue
            knowledge_res: List[Document] = knowledge.query_knowledge(query_str=query_str, **input_object.to_dict())
    ...
    def get_memory_params(self, agent_input: dict) -> dict:
        ...
        params = {"session_id": session_id, "agent_id": agent_id, "prune": prune, "top_k": top_k}
        if memory_types:
    ...
    def get_run_config(self, **kwargs) -> dict:
        llm_name = kwargs.get("llm_name") or self.agent_model.profile.get("llm_model", {}).get("name")
        callbacks = [InvokeCallbackHandler(source=self.agent_model.info.get("name"), llm_name=llm_name)]
