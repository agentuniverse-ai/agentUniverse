...
class RecursiveCharacterTextSplitter(DocProcessor):
    ...
    def splitter(self) -> Splitter:
        if not self.__splitter:
            self.__splitter = Splitter(
                separators=self.separators, chunk_size=self.chunk_size, chunk_overlap=self.chunk_overlap
            )
    ...
    def _process_docs(self, origin_docs: List[Document], query: Query = None) -> List[Document]:
        ...
        """
        lc_doc_list = self.splitter.split_documents(Document.as_langchain_list(origin_docs))
