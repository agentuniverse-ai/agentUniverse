...
class JiebaKeywordExtractor(DocProcessor):
    ...
    def _process_docs(self, origin_docs: List[Document], query: Query = None) -> List[Document]:
        ...
        for _doc in origin_docs:
            ...
            filtered_words = [
                word for word in words if word not in chinese_stopwords and word.lower() not in stop_words
            ]
            keywords = jieba.analyse.extract_tags(" ".join(filtered_words), topK=self.top_k)
