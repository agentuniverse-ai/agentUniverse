...
class WebPageReader(Reader):
    ...
    def _fetch_html(self, url: str) -> str:
        try:
            ...
            LOGGER.debug(
                "WebPageReader httpx fetch failed; falling back to requests",
                exc_info=True
            )
            try:
                ...
                raise RuntimeError(
                    f"Failed to fetch url: {url}. httpx_error={e_httpx}, "
                    f"requests_error={e_requests}"
                )
    ...
    def _extract_main_text(self, html: str, url: str) -> (str, Dict):
        ...
        try:
            ...
            LOGGER.debug("WebPageReader trafilatura extraction failed", exc_info=True)
        ...
        try:
            ...
            LOGGER.debug("WebPageReader readability extraction failed", exc_info=True)
