...
from agentuniverse.base.context.mcp_session_manager import MCPSessionManager, MCPTempClient
...
class MCPTool(Tool):
    ...
    def execute(self, **kwargs) -> CallToolResult:
        session = MCPSessionManager().get_mcp_server_session_sync(
            server_name=self.server_name, **self.get_mcp_server_connect_args()
        )
    ...
    def get_mcp_server_connect_args(self) -> dict:
        if self.transport == "sse":
            connect_args = {"transport": self.transport, "url": self.url}
        elif self.transport == "stdio":
            return {"transport": self.transport, "command": self.command, "args": self.args, "env": self.env}
        elif self.transport == "streamable_http":
            connect_args = {"transport": self.transport, "url": self.url}
        elif self.transport == "websocket":
            connect_args = {"transport": self.transport, "url": self.url}
        else:
            raise Exception(f"Unsupported mcp server type: {self.transport}")
    ...
    async def get_tool_info(self):
        async with MCPTempClient(self.get_mcp_server_connect_args()) as client:
