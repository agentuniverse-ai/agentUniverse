...
class APITool(Tool):
    ...
    def execute(self, tool_input: ToolInput):
        res = self.do_http_request(
            self.openapi_spec.get("url"), self.openapi_spec.get("method"), {}, tool_input.to_dict()
        )
        return res.text
    ...
    def convert_body_property_any_of(
    ...
    def do_http_request(
        ...
        if "requestBody" in self.openapi_spec and self.openapi_spec["requestBody"] is not None:
            ...
            if "content" in self.openapi_spec["requestBody"]:
                for content_type in self.openapi_spec["requestBody"]["content"]:
                    ...
                    body_schema = self.openapi_spec["requestBody"]["content"][content_type]["schema"]
                    required = body_schema.get("required", [])
                    properties = body_schema.get("properties", {})
                    for name, property in properties.items():
                        if name in parameters:
                            # convert type
                            body[name] = self.convert_body_property_type(property, parameters[name])
        ...
        if method in ("get", "head", "post", "put", "delete", "patch"):
            response = getattr(ssrf_proxy, method)(
                url, params=params, headers=headers, data=body, follow_redirects=True
            )
            return response
        else:
