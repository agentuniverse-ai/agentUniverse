...
class PeerAgentTemplate(AgentTemplate):
    ...
    def parse_input(self, input_object: InputObject, agent_input: dict) -> dict:
        ...
        agent_input.update(
            {"eval_threshold": self.eval_threshold, "retry_count": self.retry_count, "jump_step": self.jump_step}
        )
        return agent_input
    ...
    def execute(self, input_object: InputObject, agent_input: dict, **kwargs) -> dict:
        ...
        work_pattern_result = self.customized_execute(
            input_object=input_object, agent_input=agent_input, memory=memory, peer_work_pattern=peer_work_pattern
        )
    ...
    def customized_execute(
        self, input_object: InputObject, agent_input: dict, memory: Memory, peer_work_pattern: PeerWorkPattern, **kwargs
    ) -> dict:
    ...
    def add_peer_memory(self, peer_memory: Memory, agent_input: dict, work_pattern_result: dict):
        ...
        def _create_message_content(turn, role, agent_name, result):
            content = (
                f"Peer work pattern turn {turn + 1}: The agent responsible for {role} is: {agent_name}, "
        ...
        for i, single_turn_res in enumerate(work_pattern_result.get("result", [])):
            ...
            if planning_result:
                message_list.append(
                    _create_message_content(
                        ...
                        self.planning_agent_name,
                        planning_result.get("framework"),
                    )
                )
            ...
            if executing_result:
                message_list.append(
                    _create_message_content(
                        i,
                        "executing the specific subtask",
            ...
            if expressing_result:
                message_list.append(
                    _create_message_content(
                        i,
                        "expressing and integrating the task into a final result",
            ...
            if reviewing_result:
                message_list.append(
                    _create_message_content(
                        i,
                        "reviewing and evaluating the result",
        ...
        if self.expert_framework:
            ...
            if selector:
                ...
                if not isinstance(selector_result, dict):
                    raise ValueError(
                        "The expert framework tool selector must return a dictionary with keys"
                ...
                if not isinstance(context, dict):
                    raise ValueError(
                        "The expert framework raw context must be a dictionary with keys"
                        " for the specific content of planning, executing, expressing, and reviewing."
                    )
    ...
    def initialize_by_component_configer(self, component_configer: AgentConfiger) -> "PeerAgentTemplate":
        ...
        if self.agent_model.profile.get("expert_framework") or planner_config.get("expert_framework"):
            self.expert_framework = self.agent_model.profile.get("expert_framework") or planner_config.get(
                "expert_framework"
            )
