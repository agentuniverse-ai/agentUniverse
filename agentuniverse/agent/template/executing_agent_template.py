...
class ExecutingAgentTemplate(AgentTemplate):
    ...
    async def customized_async_execute(
        ...
        return await loop.run_in_executor(None, self._execute_tasks, input_object, agent_input, memory, llm, prompt)
    ...
    def _execute_tasks(
        ...
        if len(framework) == 0:
            return {"executing_result": [], "output_stream": input_object.get_data("output_stream", None)}

        with ThreadPoolExecutor(
            max_workers=min(len(framework), 10), thread_name_prefix="executing_agent_template"
        ) as thread_executor:
            futures = []
        ...
        return {
            "executing_result": [result for result in executing_result],
    ...
    def _execute_subtask(self, subtask, input_object, agent_input, index, memory, llm, prompt, **kwargs) -> dict:
        ...
        try:
            ...
            return {"index": index, "input": f"Question {index + 1}: {subtask}", "output": f"Answer {index + 1}: {res}"}
        finally:
    ...
    def parse_result(self, agent_result: dict) -> dict:
        ...
        stream_output(
            ...
            {
                "data": {"output": agent_result.get("executing_result"), "agent_info": self.agent_model.info},
                "type": "executing",
            },
        )
