...
class ContextualIterationAgentTemplate(AgentTemplate):
    ...
    def customized_execute(
        ...
        prompt_input_dict = {key: agent_input[key] for key in lc_prompt_template.input_variables if key in agent_input}

        chain = lc_prompt_template | llm.as_langchain_runnable(self.agent_model.llm_params()) | StrOutputParser()
        ...
        combined_res = res
        conversation_history.append({"user": lc_prompt_template.format(**prompt_input_dict), "assistant": res})
        ...
        continue_chain = (
            continue_prompt.as_langchain()
            | llm.as_langchain_runnable(self.agent_model.llm_params())
            | StrOutputParser()
        )
        if self.iteration > 1:
            if_loop_prompt: Prompt = super(AgentTemplate, self).process_prompt(
                agent_input, prompt_version=self.if_loop_prompt_version
            )
            if_loop_prompt_chain = (
                if_loop_prompt.as_langchain()
                | llm.as_langchain_runnable(self.agent_model.llm_params())
                | StrOutputParser()
            )
        ...
        for i in range(self.iteration):
            ...
            prompt_input_dict = {
                key: agent_input[key] for key in lc_prompt_template.input_variables if key in agent_input
            }
            conversation_history.append(
                {"user": lc_prompt_template.format(**prompt_input_dict), "assistant": continue_res}
            )
            agent_input["chat_history"] = json.dumps(conversation_history, ensure_ascii=False)
    ...
    async def customized_async_execute(
        ...
        for i in range(self.iteration):
            continue_res = await self.async_invoke_chain(continue_chain, agent_input, input_object, **kwargs)
            res = res + "\n" + continue_res
            ...
            if not self.if_loop_prompt_version:
                continue
            loop_res = await self.async_invoke_chain(if_loop_prompt_chain, agent_input, input_object, **kwargs)
            if not self.if_loop(loop_res):
                break
