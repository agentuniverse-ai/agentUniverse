...
class PeerWorkPattern(WorkPattern):
    ...
    async def async_invoke(self, input_object: InputObject, work_pattern_input: dict, **kwargs) -> dict:
        ...
        for _ in range(retry_count):
            peer_round_results = {}
            if not planning_result or jump_step == "planning":
                planning_result = await self._async_invoke_planning(
                    input_object, work_pattern_input, peer_round_results
                )
    ...
    async def _async_invoke_planning(
        self, input_object: InputObject, agent_input: dict, peer_round_results: dict
    ) -> dict:
        if not self.planning:
            planning_result = OutputObject({"framework": [agent_input.get("input")]})
        else:
