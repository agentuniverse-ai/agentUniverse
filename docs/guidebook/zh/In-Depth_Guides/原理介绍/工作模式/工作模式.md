...
当 `jump_step` 为 `planning` 时，若评审分数低于 `eval_threshold`，PEER 会在 `retry_count`
允许的范围内开始新一轮。下一轮中，内置 planning 智能体会接收上一轮 framework、评审分数和评审建议，
并使用这些反馈修订计划。首轮的这些可选反馈值为空；自定义 planning prompt 可以选择是否使用可选变量
`previous_planning_result`、`review_score` 和 `review_suggestion`。
