...
`ICalendarTool` 使用结构化事件数据创建、读取、检查与合并 RFC 5545 `.ics` 日历。事件支持日期或日期时间范围、IANA 时区、参与者、分类、重复规则和提醒。
...
所有路径限制在 `base_dir` 内，并限制事件数、外部载入的每个属性及属性数、参与者、分类、提醒、输入文件和生成文件大小。完整的序列化读取响应受 `max_output_chars` 限制。合并最多接受 `max_input_files` 个文件，每载入一个文件就检查累计 `max_merge_bytes` 预算，并逐步添加验证后的事件，不保留所有源日历。写入通过同目录临时文件完成，覆盖必须显式开启。
