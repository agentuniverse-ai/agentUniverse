...
`TabularDataTool` 为 CSV、TSV 和 JSONL 数据集提供有资源边界、结果确定的工作流。它支持创建、读取、列画像，以及使用结构化过滤、字段投影、数值感知排序、去重、行数限制和格式转换处理数据。
...
工具不会执行代码或表达式。路径、输入输出大小、行列数、单元格、返回上下文、画像基数和 `in` 过滤值数均受限制，写入采用原子替换。画像会统计已保留值的每次出现；当不同值数超过配置上限时，`distinct_count` 返回 `null`、同时返回真实下界，且将高频值标记为近似结果。整个序列化画像受 `max_output_chars` 限制。
